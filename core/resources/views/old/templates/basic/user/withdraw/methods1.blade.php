...
@section('content')
    <div class="text-end mb-3">
        <a class="btn btn--base" href="{{ route('user.withdraw1.history1') }}">
          <i class="las la-list"></i>  @lang('Withdraw History')
        </a>
    </div>
    <div class="card custom--card">
        <div class="card-body">
            <form action="{{ route('user.withdraw1.money1') }}" method="post" class="withdraw-form">
                @csrf
                                ...
                                @foreach ($withdrawMethod as $data)
                                    <label for="{{ titleToKey($data->name) }}"
                                        ...
                                        <div class="payment-item__thumb">
                                            <img class="payment-item__thumb-img" src="{{ getImage(getFilePath('withdrawMethod') . '/' . $data->image) }}"
