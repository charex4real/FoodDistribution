        ...
        <div class="container">
            <ul class="mr-list justify-content-center">
                <li class="mr-list__item">
                    <a class="mr-list__btn @if (!$categoryId) active @endif" href="{{ route('products1') }}">@lang('All Products')</a>
                </li>
                @foreach ($categories as $category)
                    <li class="mr-list__item">
                        <a class="mr-list__btn @if ($categoryId == $category->id) active @endif"
                            href="{{ route('products1', $category->id) }}">{{ __($category->name) }}</a>
                    </li>
                @endforeach
            </ul>
                        ...
                        <div class="product-item h-100">
                            ...
                            <div class="product-content">
                                <h6 class="product-title">
                                    <a
                                        href="{{ route('product.details1', ['id' => $product->id, 'slug' => slug($product->name)]) }}">{{ __(shortDescription($product->name, 35)) }}</a>
                                </h6>
                                ...
                                <a class="add-to-cart cmn--btn-2"
                                    href="{{ route('product.details1', ['id' => $product->id, 'slug' => slug($product->name)]) }}">@lang('Details')</a>
                            </div>
                        </div>
                    </div>
                @endforeach
