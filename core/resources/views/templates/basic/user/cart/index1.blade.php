                        ...
                        @foreach($cart->items as $item)
                            <div class="row mb-3 cart-item" data-item-id="{{ $item->id }}">
                                ...
                                <div class="col-md-4">
                                    ...
                                    <p class="text-muted">{{ showAmount($item->price) }}</p>
                                </div>
                                <div class="col-md-3">
                                    <input type="number" class="form-control quantity-input" value="{{ $item->quantity }}" min="1">
                                </div>
                                <div class="col-md-2">
                                    <strong>{{ showAmount($item->price * $item->quantity) }}</strong>
                                </div>
                                <div class="col-md-1">
                                    <button class="remm remove-item btn btn-danger btn-sm" >×</button>
                                </div>
                            </div>
                        @endforeach
                    </div>
            ...
            <div class="col-md-4 col-lg-4 order-md-last">
                <ul class="list-group mb-3">
                  <li class="list-group-item d-flex justify-content-between lh-sm">
                    ...
                    <span class="text-muted" id="cart-total" >{{ showAmount($cart->total_amount) }}</span>
                  </li>

                  
                  <li class="list-group-item d-flex justify-content-between lh-sm">
                    <div>
                      <h6 class="my-0">Wallet:</h6>
                  
                    </div>
                    <span class="text-muted">{{ showAmount(auth()->user()->wallet_balance) }}</span>
                  </li>
