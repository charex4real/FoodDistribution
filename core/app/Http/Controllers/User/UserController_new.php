...
class UserController extends Controller
{
    ...
    {   $user = User::find(auth()->id());
        ...
        if ($user->section == 1) {
            ...
            $currentUserMatrix = returnCurrent`MatrixStage(auth()->id());

            if($currentUserMatrix->stage_id == 1){
               $this->matrixService->checkStageOne($currentUserMatrix); 
            }

            if($currentUserMatrix->stage_id > 1){
                $user = User::find(auth()->id());

                $stage = MatrixStage::where('level', $currentUserMatrix->stage_id)->first();

               $this->matrixService->checkStageCompletion1($user, $stage); 
            }

            if ($currentUserMatrix->stage_id > 1 ) {

                $mat = Matrix::where('stage_id', 2)->where('user_id', auth()->id())->first();;
                if ($mat) {
            ...
            if ($currentUserMatrix->stage_id > 1 ) {
                ...
                $mat = Matrix::where('stage_id', 3)->where('user_id', auth()->id())->first();;
                if ($mat) {
                    //dd($mat);
                    $this->matrixService->reconnectImediateDownline($mat);
                }  
            ...
            if ($currentUserMatrix->stage_id > 1 ) {
                ...
                $mat = Matrix::where('stage_id', 4)->where('user_id', auth()->id())->first();;
                if ($mat) {
            ...
            if ($currentUserMatrix->stage_id > 1 ) {
                ...
                $mat = Matrix::where('stage_id', 5)->where('user_id', auth()->id())->first();;
                if ($mat) {
            ...
            if ($currentUserMatrix->stage_id > 1 ) {
                ...
                $mat = Matrix::where('stage_id', $currentUserMatrix->stage_id)->where('user_id', auth()->id())->first();;
                if ($mat) {
        ...
        return view('Template::user.dashboard', compact('pageTitle', 'totalDeposit', 'totalWithdraw', 'completeWithdraw', 'pendingWithdraw', 'totalRef', 'totalBvCut', 'total_ref', 'total_ref_debit', 'recentTransactions', 'totalShares', 'totalDividends'));
    }
    ...
    {   
        ...
        $data['amount'] = $general->registration_fee * 100;
        $data['currency'] = 'NGN';
        $trx = getTrx(20);
        $data['ref'] = $trx;
